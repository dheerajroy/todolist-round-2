...
$ python tasks.py runserver # Starts the tasks management server"""
    ...
    def render_pending_tasks(self):
        ...
        output = ['<h1>Pending Tasks:</h1>']
        for i, (key, value) in enumerate(self.current_items.items()):
            output.append(f'<p style="font-size: 20px;">{i+1}. {value} <b>[priority: {key}]</b></p>')
        if len(output) > 1:
            return '<br>'.join(output)
        return '<h1>No pending tasks.</h1>'

    def render_completed_tasks(self):
        output = ['<h1>Completed Tasks:</h1>']
        self.read_completed()
        for i, value in enumerate(self.completed_items):
            output.append(f'<p style="font-size: 20px;">{i+1}. {value}</p>')
        if len(output) > 1:
            return '<br>'.join(output)
        return '<h1>No completed tasks.</h1>'
